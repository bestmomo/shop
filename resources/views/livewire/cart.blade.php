        ...
        <x-card class="w-full md:w-3/4" title="{{ __('My cart') }}" shadow separator progress-indicator>
            ...
            @foreach ($content as $item)
                <div class="flex justify-between items-center">
                    <div class="flex gap-4 justify-between items-center mt-2">
                        <img class="w-[60px]" src="{{ asset('storage/photos/' . $item->associatedModel->image) }}"
                            alt="" />
                        <div>
                    ...
                    <div class="flex gap-2 justify-between items-center">
                        <x-input class="!w-[80px]" wire:model="quantities.{{ $item->id }}" type="number"
                            min="1" wire:change="updateQuantity({{ $item->id }})" />
            ...
            @if ($content->isNotEmpty())
                ...
                <div class="flex justify-between items-center mt-4">
                    ...
                    <x-button label="{{ auth()->check() ? __('I order') : __('Log in to order') }}"
                        icon-right="c-arrow-right" link="{{ route('order.index') }}" class="btn-primary btn-sm" />
                </div>
            @else
